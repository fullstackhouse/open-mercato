...
| `ExternalIdMappingService` (DI `externalIdMappingService`) | New method `lookupMapping(integrationId, entityType, externalId, scope, opts?)` — always present on the service type, opt-in for callers. No existing method's signature or return type changes — in particular `lookupLocalId` still returns `string \| null` | ✓ ADDITIVE (§9 DI Service Names, new method on an existing service) |
