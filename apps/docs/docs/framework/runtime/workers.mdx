...
- **It is best-effort and fires at most once.** There is no retry and no persisted intent, so a
  process that dies between the queue recording the failure and the callback finishing loses the
  report — and that is exactly when workers die. Treat it as a fast repair path, not a guarantee:
  state that must never be stranded also needs a durable check that does not depend on the queue,
  such as a scheduled sweep that fails domain rows which have stopped advancing.
