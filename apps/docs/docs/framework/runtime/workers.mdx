...
The handler therefore never runs, never throws, and never learns. Any state it created on enqueue — a
run row, a progress record — is stranded in whatever "in progress" state it was left in, and nothing
...
- Keep it small and idempotent, and make it tolerate a payload it does not recognize. It runs on the
  ...
  take the worker down, but it gets no retry.
