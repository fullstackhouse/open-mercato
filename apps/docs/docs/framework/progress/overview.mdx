...
| `touchJobHeartbeat?(jobId, ctx)` | Optional. Bump only `heartbeatAt`/`updatedAt` from a keepalive while one long unit of work is in flight. Runs on a forked entity manager, so it is safe to call while the caller's own manager is mid-transaction. Optional-chain it — third-party implementations may not provide it. |
...
**A sweep is not final.** One unit of work can legitimately outlast the timeout — a single sync batch against a slow upstream API, for example — and get swept while its producer is alive and healthy. When that producer's next progress write or `touchJobHeartbeat` arrives, the service revives the job through the same `failed → running` transition a queue retry uses, keeping the original `startedAt` so the ETA stays truthful. The revival only matches rows carrying the sweep's own `"Job stale: …"` marker, so a job that failed for a real reason keeps its status and its error message, and a producer that is genuinely dead never writes again — real sweeps stick. Prevention still beats recovery: wrap long-running units in a keepalive that calls `touchJobHeartbeat` (see `withHeartbeat` in `data_sync/lib/sync-engine.ts`).
