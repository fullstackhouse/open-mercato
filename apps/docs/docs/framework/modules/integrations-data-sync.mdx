...
async function* streamImport({ cursor, signal }: StreamImportInput): AsyncIterable<ImportBatch> {
  ...
  while (page) {
    ...
    yield { items: applied.items, cursor: applied.nextPage, hasMore: true, batchIndex: applied.index }
...
iterator on an early exit. Adapters that ignore the signal keep the previous between-batches behavior.
