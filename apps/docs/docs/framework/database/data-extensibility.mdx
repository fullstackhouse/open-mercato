...
has (last-write-wins, column left untouched). Deciding to skip is the adapter's call, not core's,
because only the adapter knows what one document comprises (header, lines, linked records) and that
applying it partially is not an option.
