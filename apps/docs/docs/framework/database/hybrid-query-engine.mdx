...
| `opts.omitAutomaticTenantOrgScope === true` | `omit_automatic_tenant_org_scope` | The caller owns scoping. `BasicQueryEngine` still handles `cf:*` filters/sorts, but the trigram search predicate, the JSONB index path, and the vector-search branch are bypassed |
...
`like`/`ilike` are re-routed to keyed trigram containment over `entity_indexes.search_trgm` when search is active, then rechecked against the decrypted rows the engine returns. If the term shapes into no trigram (under three characters once cleaned), the predicate is skipped — except on a column known to be encrypted, where it fails closed instead.
