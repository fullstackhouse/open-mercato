...
- Delete: `markDeleted()` **hard-deletes** the index row, and its trigram set with it (the set is a column on the row). The `deleted_at` column exists and readers still filter on it, but the current writers never populate it — a removed base record leaves no index row behind.
...
- Data migrations: a migration that rewrites a projected column — an `update`, or an `ADD COLUMN` with a SQL backfill — leaves `entity_indexes.doc` and the `search_trgm` derived from it on the pre-migration value forever, with nothing to signal it (no indexer job is created, and neither the record's nor the projection's `updated_at` moves). Declare the affected entity types in the migration file and `mercato db migrate` discharges the obligation after the run commits:
...
- Trigram generation is bounded per field value: the default is 20,000 input characters, tuned with `OM_SEARCH_MAX_FIELD_CHARS` (`0` disables the bound). There is no per-record cap, because the set is proportional to the record's characters rather than to a prefix expansion.
