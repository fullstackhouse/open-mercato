...
export function registerSearchModule(container: SearchContainer, options?: SearchModuleOptions): void {
  ...
  // Token strategy (always available) — Postgres entity_indexes.search_trgm via Kysely
...
|-------|---------|----------|------------|---------|
| **`entity_indexes.search_trgm`** | Postgres `int4[]` column, one GIN index | **query_index** module | The trigram builder during projection (`buildSearchTrigramHashes` in `query_index/lib/search-trigrams.ts`), written in the SAME statement as `doc` | `TokenSearchStrategy` **and directly by list-API routes** |
...
1. Synchronously updates the `entity_indexes` projection row — **including its `search_trgm` set**,
   in the same statement — so list endpoints AND list search are read-your-writes.
...
|-------------|----------|------------------------|
| **CLI** `yarn mercato search reindex` | `entity_indexes` projection (document + **`search_trgm`**) + emits **vector** events | **No** |
| **Core API** `POST /api/query_index/reindex` (feature `query_index.reindex`) | Same `reindexEntity` path — projection + trigrams + vector. `target: 'search'` in the payload (CLI: `--target search`) recomputes ONLY `search_trgm` from the stored document. | **No** |
...
- **DataTable list / column filters** — do **not** go through `SearchService`. Each list route (customers, auth users, customer_accounts, messages) queries **`entity_indexes.search_trgm` directly**, because searchable PII columns are **encrypted at rest** and `ILIKE` on ciphertext cannot match plaintext. This is why the trigram set must exist even when Meilisearch is configured.
- **Exact-match on encrypted PII** (`fieldPolicy.hashOnly`: email, phone, tax_id) — served by trigram containment plus the recheck, or by the dedicated `emailHash` column for email. Declare `fieldPolicy.kinds` (`phone`, `taxId`, `email`, `identifier`, `exact`) so these are cleaned and matched the way an operator types them.
...
Reads the keyed trigram hash set on `entity_indexes.search_trgm` via Kysely. Always available,
works with encrypted data. The strategy id is still `tokens`; only the store moved. It no longer
writes — `query_index` owns the column, so `index`/`bulkIndex`/`delete`/`purge` are no-ops.
...
- `hashOnly` — hashed into `entity_indexes.search_trgm` for match without exposing plaintext (email, phone, tax_id).
- `excluded` — never indexed anywhere (passwords, secrets, government ids).

`kinds` names how each field is cleaned and matched (`text` — the default — plus `identifier`, `phone`, `taxId`, `email`, `exact`); `kindsReplaceDefaults` narrows indexing to the declared fields only.

In addition, the trigram builder applies a global blocklist: the built-in `password,token,secret,hash` plus anything in `OM_SEARCH_FIELD_BLOCKLIST`.
...
record write
  → query_index.upsert_one (subscriber)
      → entity_indexes projection + search_trgm (sync, one statement)
...
The `OM_SEARCH_*` flags below are **trigram/Postgres-only** — they tune `entity_indexes.search_trgm` and have **no effect on Meilisearch or the vector store**:

| Variable | Default | Controls |
|----------|---------|----------|
| `OM_SEARCH_ENABLED` | `true` | Master kill-switch for trigram building and `TokenSearchStrategy`. |
| `OM_SEARCH_FIELD_BLOCKLIST` | — | Comma-separated extra field names excluded from the index (merged with built-in `password,token,secret,hash`). See below for entity-scoped entries. |
| `OM_SEARCH_MAX_FIELD_CHARS` | `20000` | Maximum input characters considered per field value before it is cut into trigrams; `0` disables the limit. |
| `OM_SEARCH_RECHECK_MAX_ROWS` | `1000` | Above this many trigram candidates the engine rechecks only the returned page and reports `totalIsApproximate`; at or below it, every candidate is rechecked and the count is exact. |
| `OM_SEARCH_DEBUG` | `false` | Verbose search/indexing debug logging (redacted — never logs raw values or PII). |

`OM_SEARCH_MIN_LEN`, `OM_SEARCH_ENABLE_PARTIAL`, `OM_SEARCH_HASH_ALGO`,
`OM_SEARCH_STORE_RAW_TOKENS`, `OM_SEARCH_MAX_TOKENS_PER_FIELD`, `OM_SEARCH_MAX_TOKENS_PER_RECORD`
and `OM_SEARCH_TOKEN_PRESENCE_CACHE_MS` were removed with `search_tokens`. A trigram is three
characters, so the minimum term length is a constant rather than a knob.

### Excluding fields from the search index
...
This matters for volume as much as for privacy: a large free-text column such as an imported
e-mail body still contributes a trigram per character to the record's set.

Changes take effect for documents built after the change — **reindex the affected entities**
(`yarn mercato query_index reindex --entity <entityType> --target search`) to drop trigrams that are already
stored.
