...
- **Any type, without a code change**: operators edit the override from the type-catalogue table on
  **Settings → Notification Delivery**, or `PATCH /api/notifications/types` with
  ...
  back to the code-declared set). The admin override replaces the code default in both directions —
  enable push for a type that ships without it, or switch any channel off entirely.
