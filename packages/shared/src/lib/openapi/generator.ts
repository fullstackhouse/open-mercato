...
function buildResponses(
  ...
  const autoErrors: OpenApiResponseDoc[] = []
  ...
  // Route-declared responses (via `responses` or `errors`) take precedence over the
  // auto-generated entries inferred from metadata: an explicit declaration is more specific
  // than a default, so a metadata-derived entry must never overwrite one the route documented
  // itself. Auto-generated entries are applied last and only for statuses the route left unset.
  for (const res of [...list, ...errorList, ...autoErrors]) {
    const status = String(res.status || 200)
    if (res.xAutoGenerated && entries[status] !== undefined) {
      continue
    }
