/**
 * Declarative field-level UI editability policy.
 *
 * A UI-read-only policy marks entity fields — or a whole entity — as
 * non-editable in the admin UI, **independent of RBAC**. It is a *display
 * policy, not a permission*, so it is enforced even for a superadmin (who
 * bypasses every feature/ACL check). It is orthogonal to, and composable
 * with, RBAC: a field can be both feature-gated AND UI-read-only.
 *
 * Entities are addressed by their canonical `module:entity` id
 * (e.g. `sales:sales_order`, `customers:customer_entity`). The value is a
 * list of read-only field ids, or the single wildcard `'*'` meaning the
 * whole entity is read-only in the UI.
 *
 * This module is pure and framework-agnostic — it is consumed both
 * server-side (override composition, optional write guard) and client-side
 * (the `@open-mercato/ui` provider/hook that CrudForm, DataTable and the
 * inline detail editors read).
 *
 * Spec: `.ai/specs/2026-07-09-declarative-ui-read-only-fields.md`
 */
 ...
 * the base declaration for that entity — mirroring the `modules.ts`
 * override contract where `null` means "disable".
 */
