 ...
 *
 * The declarative policy (`./policy`) marks fields read-only independent of
 * RBAC. This module derives a read-only map from RBAC itself: an entity is
 * marked *whole-entity* read-only in the admin UI when the current principal
 * lacks the **write feature that the entity's CRUD mutation API already
 * requires** (the `requireFeatures` on its POST/PUT/DELETE). That makes the
 * feature the write API enforces the single source of truth — the UI hides
 * edit/create/delete affordances for exactly the entities the caller cannot
 * mutate, instead of showing them and failing the save with a 403.
 *
 ...
 * `@open-mercato/ui` provider/enforcement consume.
 */
...
export type ResolveRbacReadOnlyOptions = {
  ...
  /** Registry override; defaults to the process registry. */
  registry?: CrudWriteFeatureRegistry
}

/**
 * Resolve the RBAC-driven read-only map for a principal: every registered
 * entity whose write feature the principal does not hold becomes whole-entity
 * read-only. A superadmin resolves to an empty map (fully editable) unless
 * `enforceForSuperAdmin` is set.
 */
