...
/**
 * Client-side access to the declarative UI read-only policy.
 *
 * A server component (the backend layout) resolves the effective policy map
 * with `resolveUiReadOnlyMap()` from `@open-mercato/shared/lib/ui-read-only/resolve`
 * and hands it to {@link UiReadOnlyPolicyProvider}. Editable primitives
 * (`CrudForm`, `DataTable`, the inline detail editors) then read it via
 * {@link useUiReadOnly} to render fields display-only and hide edit/add/delete
 * affordances — enforced regardless of the viewer's role (superadmin included).
 */
