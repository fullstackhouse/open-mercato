...
- **Progress writes are guarded on `status IN ('pending','running')`** — once another process finishes/cancels a job, buffered updaters stop writing to it. Exception: when a write path finds the job `failed`, it attempts exactly one revival through the start CAS (a live producer writing progress proves a stale sweep false-positived) before giving up. The revival CAS additionally matches `error_message LIKE 'Job stale:%'`, the marker `markStaleJobsFailed` writes, so a job another worker failed for a real reason keeps its status **and** its `errorMessage`/`errorStack`; `completed`/`cancelled` are never revived, and genuinely dead producers never write again, so real sweeps stick. A revival preserves the existing `started_at` — resetting it would collapse the ETA `calculateEta` derives from elapsed time; only a first `pending → running` start stamps it.
