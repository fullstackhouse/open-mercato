...
- On redelivery the progress counter is seeded from the progress job's own `processedCount`, mirroring how `committedBatches` resumes — never reset it to zero (`updateProgress` writes absolute counts) and never re-derive it from the run's `created+updated+skipped+failed`, which count emitted items and overshoot for adapters that expand one source record into several.
