...
describe('data_sync queue — abandoned job repair', () => {
  ...
  it('registers the hook on the queues whose jobs own a run row', () => {
    expect(abandonedHookFor('data-sync-import')).toEqual(expect.any(Function))
    expect(abandonedHookFor('data-sync-export')).toEqual(expect.any(Function))
  })

  it('leaves queues whose jobs carry no run id without the hook', () => {
    expect(abandonedHookFor('sync-akeneo-first-import')).toBeUndefined()
  })
