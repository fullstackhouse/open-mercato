...
type StoredOrder = ReturnType<typeof makeOrder>

function makeEm(order: StoredOrder) {
  const em: any = {
    findOne: jest.fn(async (entityClass: unknown) =>
      entityClass === SalesOrder ? order : null,
    ),
    find: jest.fn(async () => []),
...
function getHandler() {
  const handler = commandRegistry.get<DocumentUpdateInput, { order: SalesOrder }>(
    'sales.orders.update',
  )
