...
| `OM_SEARCH_RECHECK_MAX_ROWS` | When trading exactness of the list count for cost on a broad term | Default: `1000`. At or below this many trigram candidates the engine rechecks every one and the count is exact; above it only the returned page is rechecked and the response carries `totalIsApproximate`. **Token strategy only** |
| `OM_SEARCH_FIELD_BLOCKLIST` | When extra fields must never be indexed for search | Comma-separated field-name substrings, merged with built-in `password,token,secret,hash`; prefix an entry with `entityType@` to scope it to one entity. Applies to per-field values and aggregate `search_text`; run `mercato query_index reindex --target search` for affected entities after changes. **Token strategy only** |
| `OM_SEARCH_MAX_FIELD_CHARS` | When bounding large searchable values | Default: `20000`. Maximum input characters considered per field value before it is cut into trigrams; `0` disables the limit. **Token strategy only** |
| `OM_SEARCH_CUSTOMERS_INDEX_BASE_ENTITY` | When base `customers:customer_entity` rows should stay searchable | Default: `false`. A customer is found through their person/company profile, which carries the detail-page link; the base row adds only a navigation-less duplicate, and one that superadmins saw while nobody else did (#5046). `false` filters the type out of token results; `true` restores the previous behavior, where both rows are returned and the presenter enricher merges the pair. **Read-side only** — the trigram set is still written to the projection row, because `findEntityIdsBySearchTrigrams` (People/Companies list search) and the query engines' encrypted `like`/`ilike` rewrite resolve ids through it. No reindex needed either way. Policy lives in `packages/core/src/modules/query_index/lib/search-entity-policy.ts`. **Token strategy only** |
...
> The `tokens` strategy no longer owns a store: it reads `entity_indexes.search_trgm`, the keyed
> trigram hash set `query_index` writes with the projection row. `OM_SEARCH_MIN_LEN`,
> `OM_SEARCH_ENABLE_PARTIAL`, `OM_SEARCH_HASH_ALGO`, `OM_SEARCH_STORE_RAW_TOKENS` and
> `OM_SEARCH_TOKEN_PRESENCE_CACHE_MS` went with `search_tokens` — see
