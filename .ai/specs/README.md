...
| [Trigram-Hash List Search](2026-09-10-trigram-hash-list-search.md) | 2026-09-10 | List Search over Encrypted and Plain Fields — Keyed Trigram Hashes on the Projection Row | Replaces the prefix-hash `search_tokens` primitive for list search with per-tenant keyed trigram hashes stored as one `int4[]` GIN-indexed column on `entity_indexes`: substring matching over encrypted and plain fields, rechecked on decrypted rows, no plaintext, no extension, no third table; optional per-entity `searchFields` with kinds (phone, tax id, exact) |
