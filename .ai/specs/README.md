...
| [Sales External Amounts Mode](2026-09-07-sales-external-amounts-mode.md) | 2026-09-07 | Sales `external` Amounts Mode | Opt-in persisted `totals_mode` / `amounts_mode` columns under which an order mirrored from an external book of record keeps its own line amounts and its own header verbatim, including through both calculator registries; implemented in this fork, upstream decision still pending |
