...
- **UI** — both preference page clients fetch `/channels` alongside `/types`; `NotificationPreferenceMatrix` renders the fetched list instead of the hard-coded `PREFERENCE_CHANNELS`.
...
5. **Operational** — weekly range-partitioning of `push_notification_deliveries` + scheduled partition worker, GDPR data-export collectors + consumer-deletion purge, per-device locale resolution with title/body rewrite, cross-user token-handoff deactivation, and **Expo receipt polling** (`getPushNotificationReceiptsAsync`): Expo's `DeviceNotRegistered` for a stale-but-well-formed token surfaces in the receipt phase, not the ticket the adapter returns synchronously, so the common "app uninstalled" case is not yet soft-deleted for Expo — a receipt-polling pass is needed to close it (see the limitation comment in `channel-expo/lib/adapter.ts`).
6. **Actionable-notification action buttons (`pushOptions.category` / cross-platform `actions`)** — clickable buttons on the delivered push. The two platforms differ fundamentally, so this needs its own design pass:
   - **iOS (APNs):** OS-native. Payload carries `aps.category = "<id>"`; the buttons are registered **client-side** as a `UNNotificationCategory`/`UNNotificationAction` under that id. Backend controls only *which* category. Minimal upstream change: a recognized `pushOptions.category` mapped to `aps.category` in the APNs adapter and FCM's `apns.payload.aps.category`, plus Expo `categoryId`.
   - **Android (FCM):** **no** OS-level category→buttons in the payload. Buttons are built by the app from a **data message** (`NotificationCompat.Builder.addAction` + `PendingIntent`), which for background display in React Native needs a local-notification library (e.g. Notifee). So parity requires echoing the identifier into the FCM `data` payload (e.g. `data.category`) for the client handler to read — the `aps.category`-only mapping does not reach Android.
   - **Why deferred:** a clean cross-platform contract (single `category` id vs a richer `actions: [{id,label,icon}]` carried in `data` for Android while iOS/labels live client-side), plus the mandatory **client-side registration** (iOS categories, Android Notifee handler), means this is not a backend-only change. It also **cannot be verified end-to-end** in the current test harness (Android emulator + FCM only; no iOS simulator, no connected APNs `.p8` channel, and the reference app registers no categories). Prototyped on 2026-07-02 (mapping + unit tests across all three adapters) then reverted to keep Phase 5 payload-only; to be reintroduced with the client-side work and an iOS/APNs e2e path.
