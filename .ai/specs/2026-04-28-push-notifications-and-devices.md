...
**Status:** In progress — Phase 1 (`devices` module) complete (`TC-DEV-001` + `TC-DEV-002` colocated); **Phase 2 (`notifications` extensions) complete** (`TC-NOTIF-011` colocated; type catalogue + preferences on `feat/devices-push-e2e`); **Phase 3 (`push_notifications` rails + delivery-log UI) complete** (`push` delivery strategy + delivery log + `send-push` worker + `push_stub` adapter + read-only delivery-log API/UI; `push-delivery`/`push-delivery-strategy` unit suites + `TC-PUSH-001` API suite colocated); **Phase 4 (reference provider adapters) complete** (`packages/channel-{fcm,apns,expo}` hub `ChannelAdapter` packages — `firebase-admin` / `@parse/node-apn` / `expo-server-sdk` — uniform `device_unregistered` sentinel, shared `pushChannelCapabilities` baseline + `readPushEnvelope` helper, per-device provider routing in the strategy; per-adapter unit suites colocated); **Phase 5 (silent push + flexible push payload) complete** (`sendSilentPush` server primitive gated on a `silent: true` notification type + no `Notification` row + no preference check; shared `fanOutPushDeliveries`; arbitrary `data` + flat `pushOptions` on every notification, mapped per provider; `silent`/`options` on the push envelope; additive across `notifications`/`push_notifications`/the three channel adapters); Phase 6 pending. **Module 3 architecture revised 2026-06-25** to deliver push **through the `communication_channels` hub** (FCM/APNs/Expo as hub `ChannelAdapter`s) per maintainer request on PR #2595 — see "Module 3" below and the 2026-06-25 changelog entry. Targeting `develop` as a single end-to-end PR (Phases 1–5 together; fullstackhouse PR #10) — revised 2026-06-29, see "Implementation Phases" + changelog; Phase 6 follows separately.
**Date:** 2026-04-28
...
4. **Phase 4 — Reference provider adapters (hub channel packages).** *(Complete.)* `packages/channel-fcm` (`firebase-admin`), `packages/channel-apns` (`@parse/node-apn`, native HTTP/2 + token `.p8`), `packages/channel-expo` (`expo-server-sdk`) — each a `ChannelAdapter` package mirroring `channel-gmail`: credentials schema, health check, uniform `device_unregistered` sentinel, SDK isolated behind a test seam, provider client cached per credentials hash. Shared `pushChannelCapabilities` baseline + `readPushEnvelope` helper added to `communication_channels/lib` (stub refactored to reuse the baseline). Per-device provider routing added to the push strategy (`device.pushProvider` → matching channel). **No new admin UI** — operators connect a push provider through the existing `communication_channels` credentials-connect flow. **APNs HTTP/2 risk retired** by `@parse/node-apn` (the transport the downstream covo build runs in prod). **BC:** 3 new provider packages + 3 module ids (`channel_{fcm,apns,expo}`), new `channel_{fcm,apns,expo}.{view,configure}` ACL features, 2 additive `communication_channels/lib` helpers — all additive; the `ChannelAdapter` contract is unchanged.
5. **Phase 5 — Silent push + flexible push payload.** *(Implemented.)* The original Phase-5 admin/preferences UI was dissolved into Phases 2–3; the slot is reused for the highest-priority deferred item. (a) **Silent push:** `pushNotificationService.sendSilentPush({ resolve, tenantId, userId, type, data?, pushOptions? })` (`push_notifications/lib/send-silent-push.ts`, registered in `di.ts`) delivers a content-available wake-up to one user's devices. *(Originally: **no** in-app `Notification` row and **no** preference check — **superseded 2026-06-30**, see Changelog: it now routes through `notificationService.create()`, persisting a row and respecting preferences; force with `nonOptOut: true`.)* Silent-ness is a property of the registered notification **type** (`NotificationTypeDefinition.silent: true`; mirrored to a `notification_types.silent` column as of 2026-06-26 — see Changelog), validated on every call; it is never a per-call flag. The shared device/channel fan-out is extracted into `lib/push-fanout.ts` (`fanOutPushDeliveries`) and reused by both the `push` strategy and `sendSilentPush`. A `silent` boolean snapshot column is added to `push_notification_deliveries`. (b) **Flexible payload:** notifications gain optional `data` (arbitrary app-readable string map — persisted, exposed in the DTO, delivered in the push data payload) and `pushOptions` (flat `sound`/`badge`/`image`/`priority`/`channelId`/`body` map — persisted, push-only). The push envelope (`communication_channels/lib/push-envelope.ts`) carries `options`/`silent`; the FCM/APNs/Expo adapters branch on `silent` (data-only content-available) and map `pushOptions` onto each provider's native message. **No new HTTP route / ACL / event** — silent push is a server-side primitive other modules trigger via DI (e.g. event subscribers). **BC:** additive `silent` on `NotificationTypeDefinition`; additive `data`/`pushOptions` on the create schemas + `Notification` entity (2 nullable JSON columns); additive `silent` column on `push_notification_deliveries`; additive `options`/`silent` on `PushEnvelope`; new DI name `pushNotificationService` — all additive, no contract break. Tests: `send-silent-push` + extended `push-delivery-strategy`/`push-delivery`/`push-envelope` + per-adapter silent/options unit suites; `TC-NOTIF-012` integration (visible `data`/`pushOptions` round-trip). Silent push has no HTTP trigger, so it is unit-covered + manually verified end-to-end.
...
> **Status: not started — separate additive spec/PR.** This is the one architectural gap surfaced by the 2026-06-29 end-to-end review. It is intentionally **out of scope for this PR** because it touches the existing in-app/email runtime and carries regression risk that should not ride on the push feature.
...
2. **Lift per-channel preference + `nonOptOut`/`silent` enforcement into one shared gate** (e.g. `shouldDeliver(type, channel, scope)`) invoked by the dispatcher before every `strategy.deliver(ctx)`, instead of each strategy re-implementing (and potentially forgetting) opt-out logic. Closes the silent in-app/email enforcement gap.
...
**BC:** all four are additive (new strategy registrations, a new optional gate helper, an optional capability hook, an additive context split with a deprecation bridge for the email-shaped fields). Behavior change is intentional and corrective: `in_app`/email preferences and `nonOptOut`/`silent` start being enforced where they silently were not. Requires its own spec entry + integration coverage for in-app/email opt-out before landing.

## Pre-PR Hygiene Checklist

Must be cleared before opening the upstream PR (against `develop`):

- [ ] **Move `docs-devices-and-push-getting-started.md` out of the repo root** into `apps/docs/` (or drop it from the PR). It is branch-introduced and violates the "no stray files at repo root" convention.
...
- [ ] **Reconcile the `TC-DEV-002` reference** in this spec with the shipped suites (`TC-DEV-001` self-serve + admin tree; `TC-DEV-003`). Either correct the spec text or add the missing suite.
- [ ] **Verify channel-package version ranges resolve at publish time.** `channel-fcm`/`channel-apns`/`channel-expo` are at `0.1.0` while `core`/`shared` are `0.6.x`; confirm their `@open-mercato/core` (dep) and `@open-mercato/shared` (peer) `workspace:*` ranges publish to concrete ranges a fork can install, or `yarn install` downstream will fail.
...
- **2026-06-30** — **`sendSilentPush` removed; silent push is now just the normal notification flow** on `unify-silent-with-visible-uctbt`. Per maintainer request, there is no dedicated silent-push primitive at all. A silent push is simply a notification whose **type** is declared `silent: true`, created through the ordinary `notificationService.create()` path — exactly the same flow as a visible push. The `push` delivery strategy already derives `silent` from the type, sends a content-available (data-only) push, and skips user-facing copy; the in-app `Notification` row is still created and per-channel preferences still apply (force with `nonOptOut: true`). **Removed:** `push_notifications/lib/send-silent-push.ts` + its unit suite. The DI service object `pushNotificationService` (now exposing only `sendCustomPush`) + the `PushNotificationService` interface moved to `lib/send-custom-push.ts`; `di.ts` + `api/custom-send/route.ts` imports updated. **Reverted:** the interim `deliver-notification.ts` email guard for silent types — per-channel respect-of-`silent` (email/in-app) is deferred to the Phase 7 channel-seam work, not handled here. `sendCustomPush` (visible admin one-off) is unchanged. This supersedes the earlier same-day "wrapper" approach and the 2026-06-29 / Phase 5 "no `Notification` row / no preference check" statements. **BC:** none — the entire `push_notifications` feature (incl. the now-deleted `sendSilentPush`) is unreleased; the `pushNotificationService` DI name is unchanged. Validation: `send-custom-push` + `deliver-notification` + `push-delivery-strategy` + `push-delivery` + `push-fanout` suites green; `grep sendSilentPush` → no matches in `src`.
- **2026-06-29** — **End-to-end review findings folded into the spec (no code change).** A four-track audit (push-as-channel design, backward-compatibility, OSS packaging/reusability, contributing-guide compliance) confirmed the branch is BC-safe (all-additive, no breaking contract-surface change) and reusable downstream (correct dependency direction, no illegal cross-module ORM relations, provider SDKs isolated in their packages, tenant scoping + optimistic locking in place). Two documentation/scope additions captured the outcomes: (1) a dedicated **"Migration & Backward Compatibility"** section (additive-only inventory + fork migration safety); (2) **"Phase 7 — Unify all channels on the delivery-strategy seam"**, recording the one architectural gap — in-app and email bypass the `NotificationDeliveryStrategy` seam (email is inlined in `deliver-notification.ts`, in-app is implicit row creation), so per-channel preference + `nonOptOut`/`silent` enforcement currently fires only on push; planned as a separate additive spec/PR to avoid in-app/email regression risk on this PR. Also added a **"Pre-PR Hygiene Checklist"** (relocate the root-level `docs-devices-and-push-getting-started.md` into `apps/docs/`, purge the pre-existing `report-high.md` ops artifact, reconcile the stale `TC-DEV-002` reference, verify channel-package publish version ranges). Documentation/scope only.
- **2026-06-29** — **`silent` types are now user-disablable (silent decoupled from preference bypass)** on `feat/devices-push-e2e`. Previously the `push` delivery strategy treated `silent` as a second opt-out bypass alongside `nonOptOut` (per the 2026-06-26 entries below) — a silent type's push could not be turned off. `silent` now controls **only** delivery style (background content-available wake-up vs visible alert), not enforcement: the strategy (`push_notifications/lib/push-delivery-strategy.ts`) gates **every** type on the recipient's per-channel `isChannelEnabled` preference and bypasses it **only** when `type.nonOptOut === true`. A silent type that must stay forced now declares `nonOptOut: true` (the shipped `admin.custom_message` already does, so it is unaffected). No catalogue/UI change was needed: `GET /api/notifications/types` already keys settings visibility off `hiddenFromSettings` (independent of `silent`), and `setPreferences` already permits opt-out rows for non-`nonOptOut` types — so a silent type that omits `hiddenFromSettings` appears in the preferences screen and can be toggled. **Scope:** the change is the normal `notificationService.create`/`createBatch` flow only; the explicit `sendSilentPush()` helper remains a forced background wake-up (no `Notification` row, no preference check) by design. **BC:** behavior change for any silent-but-not-`nonOptOut` type — its push now respects opt-out where it previously always fired; the only shipped silent type is `nonOptOut` and is unchanged. Tests: replaced the `push-delivery-strategy` "silent bypasses preferences" case with three — silent+enabled delivers as silent, silent+opted-out skips, silent+`nonOptOut` forces. Validation: `push-delivery-strategy` suite green (13).
- **2026-06-29** — **Delivery model revised: single end-to-end PR (supersedes "one PR per phase").** Phases 1–5 ship together on `feat/devices-push-e2e` as one PR against `develop` (fullstackhouse PR #10), not the originally-planned per-phase PRs. Updated the **Status** line and the **Implementation Phases** intro accordingly; the phase breakdown stays the unit of work/review but the branch is not split. Phase 6 (purge worker, web push, additional providers) follows as a separate PR. Documentation/scope only — no code change.
