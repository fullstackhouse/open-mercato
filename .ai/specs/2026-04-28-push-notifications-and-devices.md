...
- **2026-07-01** — Phase 3 maintainer review (PR #18, patrykbojczuk — approved with two comments). (1) **Valid schedule UUID** — `setup.ts` `stableScheduleUuid` produced a uuid-*shaped* sha256 slice without forcing the RFC-4122 version/variant nibbles; the scheduler validates schedule ids with zod's strict `z.uuid()` (which this repo enforces — see the earlier TC-PUSH-001 404-vs-400 note), so the raw hash could be rejected. Now folds the hash into a valid v5-style (name-based) UUID (version nibble `5`, variant nibble `8..b`); exported + unit-tested (`__tests__/setup.test.ts`) that every derived id passes `z.string().uuid()`, is deterministic, and is per-key distinct. (The `communication_channels` poll-tick registration it was copied from has the same latent flaw — a separate follow-up, out of scope here.) (2) **`next_retry_at` in `deliveryListFields`** — already resolved in the prior review-round commit (added to the list projection), matching the same drift the automated review flagged.
