...
1. ~~**Silent push** (highest priority)~~ — **implemented in Phase 5** (see Implementation Phases). `sendSilentPush`: content-available wake-ups that create no `Notification` row and bypass preferences; gated on a `silent: true` type. Frequency caps remain deferred (item 3).
