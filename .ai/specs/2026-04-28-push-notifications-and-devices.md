...
**2b′. Operator-editable channel eligibility.** The per-type eligibility field `NotificationTypeDefinition.channels` (gate step 2 — checked before both the `nonOptOut` bypass and user preferences, so a channel outside the set is completely off for the type) gains an operator override: `notification_types.channels` (nullable JSONB the catalogue sync never mirrors), edited via `PATCH /api/notifications/types` or the type-catalogue table on the Notification Delivery settings page. A stored array **replaces** the code-declared set; `NULL` inherits it; effective = `stored ?? code ?? all channels`. Every built-in type declares `channels: ['in_app', 'email']` — push ships off so a freshly connected push provider does not flood devices with catalogue-wide pushes — and an operator re-enables push per type without a code change. For a channel outside the effective set, `setPreferences` drops writes server-side and both preference UIs render the cell locked off; channels inside the set behave exactly as before (user preferences, absent row ⇒ enabled). `GET /api/notifications/types` exposes `channels` (effective) and `storedChannels` (raw override) so UIs lock exactly the cells delivery rejects. The `nonOptOut` flag follows the same contract: the mirrored `non_opt_out` column became operator-owned (`NULL` = inherit the code declaration, never re-mirrored by the sync; existing mirrored values are nulled by the migration), editable from the same PATCH/settings surface — `true` forces a type on for users, `false` makes a code-required type user-editable; `GET` returns the effective `nonOptOut` plus `storedNonOptOut`.
...
| Wiring or removing `channel_{fcm,apns,expo}.{view,configure}` ACL features | Granted in each package's `setup.ts` but never consulted by a `requireFeatures` guard — the connect flow uses `communication_channels.connect_tenant_channel`. Introduced by this PR, so not yet a BC concern. |
...
### 2026-07-16 (later)
- Operator-editable channel eligibility (§ 2b′): `notification_types.channels` (nullable JSONB, PATCH `/api/notifications/types` + settings-page catalogue) overrides the code-declared `NotificationTypeDefinition.channels`; a channel outside the effective set is completely off for the type (beats user preferences and `nonOptOut`; preference UIs lock the cell, `setPreferences` drops writes server-side). All 28 built-in types now declare `channels: ['in_app', 'email']` — push is re-enabled per type from the adminka. `nonOptOut` follows the same operator-owned contract (mirrored column became a nullable override; sync no longer mirrors it). `admin.custom_message`/`admin.custom_silent` stay unrestricted.
