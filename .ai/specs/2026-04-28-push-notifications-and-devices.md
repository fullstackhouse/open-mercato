...
- **Fixed the CI integration failure in `TC-CHANNEL-PUSH-005..007`.** All three real-delivery specs timed out at exactly 60s on every attempt in CI while passing locally. They relied on `test.slow()`, which only triples the config's `timeout: 20_000` — a budget their two queue drains plus a 30s poll exhaust on a loaded runner. Same defect and same fix as `TC-PUSH-005`/`009` on 2026-07-09 (`test.setTimeout(120_000)`); the three channel-package specs were missed then. Test-only; the shipped code was never at fault. The registration specs (`001..004`), which need no worker, passed throughout.
